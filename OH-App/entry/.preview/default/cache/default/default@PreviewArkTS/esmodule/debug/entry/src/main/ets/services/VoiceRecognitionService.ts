...
import { VOICE_CONFIG, VOICE_COMMANDS } from "@normalized:N&&&entry/src/main/ets/constants/RobotConstants&";
...
export class VoiceRecognitionService {
    ...
    async checkPermission(): Promise<boolean> {
        try {
            ...
            const result = await atManager.checkAccessToken(0, 'ohos.permission.MICROPHONE');
    ...
    async requestPermission(): Promise<boolean> {
        try {
            const atManager = abilityAccessCtrl.createAtManager();
            const permissions: Array<Permissions> = ['ohos.permission.MICROPHONE'];
            const result = await atManager.requestPermissionsFromUser(0, permissions);
