import type { RobotStatus, MonitorData } from '../models/RobotModels';
...
export class VoiceErrorHandler {
    ...
    static handleRecognitionError(error: any): string {
        if (error.code === -1) {
            return '语音识别失败，请重试';
        }
        else if (error.code === -2) {
...
export class CommandErrorHandler {
    ...
    static handleExecutionError(error: any): string {
        if (error.msg) {
            return error.msg;
        }
        return '指令执行失败，请重试';
