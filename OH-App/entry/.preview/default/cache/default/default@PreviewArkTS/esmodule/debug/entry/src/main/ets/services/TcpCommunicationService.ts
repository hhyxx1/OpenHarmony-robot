...
import type { Message, CommandResponse, ControlCommand } from '../models/RobotModels';
import { NETWORK_CONFIG } from "@normalized:N&&&entry/src/main/ets/constants/RobotConstants&";
...
export class TcpCommunicationService {
    ...
    private setupMessageListener(): void {
        ...
        this.tcpSocket.on('message', (data) => {
            try {
                const message = JSON.parse(data.message.toString('utf-8')) as Message;
                this.handleMessage(message);
            }
            catch (error) {
                console.error('解析消息失败:', error);
                this.callback?.onError('数据格式异常');
            }
        });
        this.tcpSocket.on('error', (error) => {
            console.error('TCP错误:', error);
            this.isConnected = false;
            this.callback?.onError(`网络错误: ${error}`);
        });
